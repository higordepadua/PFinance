 ...
 * prefix to shows to the binder the fields must be to each class.
 * If you setName with "[pfx:cads]" you need pass cads to map only the cads fields.
 * 
 * Example:
 * jTextField.setName("[name:personName]")
 * jTextField2.setName("[name:age][fmt:int]");
 * binder = new Binder(container, Person.class,new IntFormatter());
 * binder.updateView(personObject);
 * //change the fields values
 * binder.updateModel(personObject);
 * 
 * jTextField.setName("[pfx:person][name:personName]")
 * jTextField2.setName("[pfx:cia][name:name]");
 * 
 * personBinder = new Binder("person",container,Person.class);
 * ciaBinder = new Binder("cia",container, Cia.class);
 * 
